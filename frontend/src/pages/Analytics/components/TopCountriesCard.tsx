							...
							<div key={country.countryCode} className="flex items-center justify-between gap-3">
								<div className="flex items-center gap-2 min-w-0">
									...
									<div className="min-w-0">
										<p className="text-sm font-medium truncate">{countryCodeToName(country.countryCode)}</p>
										<p className="text-xs text-muted-foreground">{country.countryCode}</p>
									</div>
								</div>
								<div className="flex items-center gap-3 min-w-[160px]">
									<div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
										<div className="h-full bg-cyan-500" style={{ width: `${Math.max(4, (country.count / maxCount) * 100)}%` }} />
									</div>
									<div className="text-right">
										<p className="text-sm font-medium">{country.count.toLocaleString("de-DE")}</p>
										{typeof country.percentage === "number" ? (
											<p className="text-xs text-muted-foreground">{country.percentage.toFixed(1).replace(".", ",")}%</p>
										) : null}
									</div>
								</div>
							</div>
